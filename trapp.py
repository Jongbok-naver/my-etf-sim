...
if st.button("🚀 시뮬레이션 시작", use_container_width=True):
    ...
    if not valid_configs:
        ...
        with st.spinner("데이터 로딩 및 계산 중..."):
            ...
            if months_diff < 1: months_diff = 1 # 최소 1개월
            ...
            for config in valid_configs:
                ...
                if price is None:
                    st.error(f"❌ '{config['name']}({config['code']})'의 가격 데이터를 가져오지 못했습니다. 티커를 확인하거나 잠시 후 다시 시도하세요.")
                    continue
